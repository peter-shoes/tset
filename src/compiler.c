  ...
  while (!is_stack_empty ())
    {
      ...
      if ((pop.type == TOKEN_DEF) || (pop.type == TOKEN_MATHDEF))
        if ((peek_next (0).type == TOKEN_MACRO)
          ...
          {
            macro_track_t *tail = (pop.type == TOKEN_DEF) ? def_track_tail
                                                          : mathdef_track_tail;
            tail->macro = peek_next (0);
            tail->body = peek_next (1);
            tail->next = (macro_track_t *)malloc (sizeof (macro_track_t));
      ...
      if ((pop.type == TOKEN_WORD) || (pop.type == TOKEN_MATHWORD))
        {
          ...
          macro_track_t *tmp
              = (pop.type == TOKEN_WORD) ? def_track : mathdef_track;
          macro_track_t *tail
              = (pop.type == TOKEN_WORD) ? def_track_tail : mathdef_track_tail;
          while (tmp != tail)
            {
